...
def dict_to_bytestr(the_dict):
    '''Convert a dict to a base64 encoded string to attach it to
    a pub/sub message.

    Inputs
    ------
    the_dict : dict
        Dictionary to convert to base64 encoded string

    Returns
    -------
    bytestr_dict : bytestr
        base64 encoded string version of the dictionary
...
def gen_publisher_and_path(project_id, topic_id):
    '''Generate a publisher client and published path for pub/sub
    messaging.
...
def gen_brick_batch_uuid(brick_type=None):
    '''Create a unique UUID for batches of bricks.
    ...
    Returns
    -------
    brick_batch_uuid : str
        UUID for a toto brick batch
