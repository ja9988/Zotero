...
def get_fof_bs4_page_limit(username, page, num_friends_per_page, timeout_lim=20, metadata_requests=None, how='username'):
    print('GETTING_FOLLOWINGS-WOULD_BE_ONE_API_REQUEST')
    all_page_metadata = get_fof_bs4_page(username, page, timeout_lim=20, metadata_requests=None, how=how)
    friends_metadata = all_page_metadata['metadata']
    num_friends_returned = len(friends_metadata.keys())
    if num_friends_returned > num_friends_per_page:
        metadata_temp = {}
        list_parent_metadata_temp = list(friends_metadata.items())[0:num_friends_per_page]
        for entry in list_parent_metadata_temp:
            metadata_temp[entry[0]] = entry[1]
    else:
        metadata_temp = friends_metadata

    return metadata_temp

def get_followers_bs4_page_limit(username, page, num_followers_per_page, timeout_lim=20, metadata_requests=None):
    print('GETTING_FRIENDS-WOULD_BE_ONE_API_REQUEST')
    all_page_metadata = get_followers_bs4_page(username, page, timeout_lim=20, metadata_requests=None)
    followers_metadata = all_page_metadata['metadata']
    num_followers_returned = len(followers_metadata.keys())
    if num_followers_returned > num_followers_per_page:
        metadata_temp = {}
        list_parent_metadata_temp = list(followers_metadata.items())[0:num_followers_per_page]
        for entry in list_parent_metadata_temp:
            metadata_temp[entry[0]] = entry[1]
    else:
        metadata_temp = followers_metadata

    return metadata_temp


def get_fof_bs4_page(username, page, timeout_lim=20, metadata_requests=None, how='username'):
    ...
    for attempt in range(6):
        if attempt > 0:
            print(f"more_than_one_attempt {attempt} for {username} page {page}")
        try:
            # Submit GET request
            ...
            if attempt > 3:
                print(f'failed_to_load_page {page} for {username}. Will retry... attempt {attempt} logged, {e}')
        ...
        print(f'total_failure_getting page {page} for {username}')
    friends_metadata = {}
    ...
    try:
        ...
        for friend_soup in friends_soup:
            ...
            metadata = _username_char_ind(metadata, how=how)
            friends_metadata[child_username] = metadata
    except Exception as e:
        print(f"missing_page {page} for {username}! {e}")
        pass

    if len(friends_metadata) == 0:
        print(f"problem_getting_metadata_page {page} for {username} metadata: {friends_metadata}")

    return {'parent': username, 'page': page, 'metadata': friends_metadata}


def get_followers_bs4_page(username, page, timeout_lim=20, metadata_requests=None):
    '''Function to get metadata from friends list view page.
    ...
    # Build URL to get 50 most recent friends
    whotwi_link = f"https://en.whotwi.com/{username}/followers/user_list?page={page}+&view_type=list"
    # Get page of friends
    for attempt in range(6):
        if attempt > 0:
            print(f"more_than_one_attempt {attempt} for {username} page {page}")
    ...
    try:
        followers_soup = whotwi_soup.find_all('li', itemtype="http://schema.org/Person")
        for follower_soup in followers_soup:
            # create dictionary for metadata from default type for fof
            metadata = MetadataDefaults().default_fof
            child_username, metadata = _username_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _userid_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _list_name_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _counts_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _twitter_url(child_username, metadata, metadata_requests)
            metadata = _bio_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _website_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _last_tweet_friends_of_friends(follower_soup, metadata, metadata_requests)
            metadata = _last_checked_friends_of_friends(metadata, metadata_requests)
            metadata = _parent_account_friends_of_friends(username, metadata, metadata_requests)
            metadata = _username_char_ind_parent(metadata)
            followers_metadata[child_username] = metadata
    except Exception as e:
        print(f"missing_page {page} for {username}! {e}")
        pass

    if len(followers_metadata) == 0:
        print(f"problem_getting_metadata_page_followers {page} for {username} metadata: {followers_metadata}")

    return {'parent': username, 'page': page, 'metadata': followers_metadata}
...
def _username_char_ind(metadata, how='username'):
    # how flag added so that master_friends table
    # is efficiently queryable by parent_account
    if how == 'username':
        username = metadata['username']
        char_ind = letter_to_num(username)[0]
    elif how == 'parent_account':
        username = metadata['parent_account']
        char_ind = letter_to_num(username)[0]

    metadata['username_char_ind'] = char_ind

    return(metadata)

def _username_char_ind_parent(metadata):
    # this is so master_followers can be
    # queried by parent_account efficiently
    username = metadata['parent_account']
    char_ind = letter_to_num(username)[0]
    metadata['username_char_ind'] = char_ind
...
def _last_tweet_friends_of_friends(friend_soup, metadata, metadata_requests):
    if metadata_requests['last_tweet'] is True:
        ...
        try:
            ...
            if 'minute' in tweet_time_str:
                metadata['last_tweet'] = time_now - relativedelta(minutes=tweet_time_number)
